...
def test_cube_loop_replacement():
    ...
    """
    gen = magpy.magnet.Cuboid(polarization=(1, 2, 3), dimension=(1, 1, 1))
    cube = magpy.magnet.Cuboid(
        polarization=(0, 0, 1), dimension=(2, 2, 2), position=(1, 2, 3)
    )
...
def test_physics_at_distance():
    ...
    cube = magpy.magnet.Cuboid(dimension=(1, 1, 1), polarization=J, position=pos)
    cube.meshing = 100
    ...
    for i in range(1, 3):
        errF = abs(
            np.linalg.norm(FT[0, 0] - FT[i, 0]) / np.linalg.norm(FT[0, 0] + FT[i, 0])
        )
        errT = abs(
